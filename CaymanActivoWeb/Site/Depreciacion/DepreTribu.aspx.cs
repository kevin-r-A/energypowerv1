...
public partial class DepreTribu : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {            
            this.Title = ConfigurationManager.AppSettings["Titulo"] + " Generar Depreciación TRIBUTARIA";
            cargarmeses();
        }
    }
    ...
    {
        ddmeses.Items.Clear();
        DateTime nextDepre = Logica.HELPER.getMesDepre("TRIBUTARIA");
        ...
        oldDate = oldDate.AddDays(-oldDate.Day).AddDays(28);
    ...
    protected void ibbus1_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            if (Logica.HELPER.getEstadoDepreciacion("TRIBUTARIA") == "OUT") //SI NO HAY PROCESOS EN OTRA PAGINA WEB DEPRECIANDO LOS ACTIVOS
            {
                Logica.HELPER.insDepreciacion(Convert.ToDateTime(ddmeses.SelectedValue), "TRIBUTARIA", Membership.GetUser().UserName.ToLower());
    ...
    {
        rgdepre.ExportSettings.FileName = "TRIBUTARIA_"+ddmeses.SelectedItem.Text.Replace(" ","");
    ...
    {
        //if (e.Worksheet.Table.Rows.Count == rgdepre.Items.Count + 1)
        //{

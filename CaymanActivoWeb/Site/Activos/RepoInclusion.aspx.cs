...
public partial class RepoCus : System.Web.UI.Page
{
    ...
    {
        this.Title = ConfigurationManager.AppSettings["Titulo"] + " Reporte Inclusión de Seguros";
    ...
    public DataTable setExport(DataTable dt)
    {
        ...
        try
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row["Código"].ToString() != "")
                {
                    row["Código"] = "'" + row["Código"].ToString().ToUpper();
                }
                else
                {
                    row["VALOR COMPRA"] = "  ";
    ...
    public void exportarPdf(GridView grid, string nombre)
    {
        try
        {
            ...
            Document document = new Document(iTextSharp.text.PageSize.A4.Rotate(), 25, 15, 15, 5);
            ...
            for (int rowNo = 0; rowNo < noOfRows; rowNo++)
            {
                ...
                mainTable.SetWidths(new Single[] { 80, 60, 70, 70, 70, 70, 70, 60, 50, 70, 70,70,70 });
            }
            ...
            string Path = Server.MapPath("./PDF/") + nombre + "INCLUSION" + System.DateTime.Now.ToString("ddMMyyyyHHmmss") + ".pdf";
            ...
            //Paragraph P = new Paragraph("REPORTE DE ACTIVOS ASIGNADOS POR CUSTODIO \n\n", myfont2);
            //P.Alignment = Element.ALIGN_CENTER;
            //document.Add(P);
            //document.Add(new Paragraph("Fecha: " + System.DateTime.Now.ToString() + "\n ", myfont));
            //document.Add(new Paragraph("Custodio: " + CUSTODIO + "\n\n ", myfont));
            ...
            jpg.ScalePercent(40f); //tamaño de imagen en porcentaje
            jpg.Alignment = iTextSharp.text.Image.ALIGN_RIGHT;

            //dice
            string AreaActivos = ConfigurationManager.AppSettings["AreaActivosFijosIngreso"].ToString();
            string TituloReporte = ConfigurationManager.AppSettings["TituloReportes"].ToString();
            Paragraph P = new Paragraph("ACTIVOS FIJOS Y BIENES SUJETOS DE CONTROL -  INCLUSION DE SEGUROS #" + PDFnum + "\n", myfont2);
            P.Alignment = Element.ALIGN_CENTER;
            document.Add(jpg);
            document.Add(P);

            Paragraph Pfd = new Paragraph(TituloReporte + "\n", myfont2);
            Pfd.Alignment = Element.ALIGN_CENTER;
            document.Add(Pfd);

            document.Add(new Paragraph("\n"));

            Paragraph P1 = new Paragraph("FECHA DE CORTE AL "  + System.DateTime.Now.ToString("dd-MM-yyyy") + "\n ", myfont);
            P1.Alignment = Element.ALIGN_JUSTIFIED;
            document.Add(P1);

            //Paragraph P2 = new Paragraph("En la ciudad de " + ddUge2.SelectedItem.ToString() + " ,los suscritos señor(a)  " + AreaActivos.ToString() + ",  quien entrega los bienes,  señor(a)  " + cust1 + "  quien  recibe los bienes, nos constituimos en las oficinas de " + Session["emid"].ToString() + ", ubicadas en la siguiente dirección: " + ddUge1.SelectedItem.ToString() + ", " + ddUge2.SelectedItem.ToString() + ", " + ddUge3.SelectedItem.ToString() + ", " + ddPiso.SelectedItem.ToString() + ",  con el objeto de realizar la diligencia de entrega – recepción  correspondiente. Al efecto con la presencia de las personas mencionadas anteriormente se procede con la constatación física y entrega-recepción de los activos fijos y bienes sujetos de control – inclusión de seguros, de acuerdo al siguiente detalle: \n ", myfont);
            //P2.Alignment = Element.ALIGN_JUSTIFIED;
            //document.Add(P2);

            //Paragraph P3 = new Paragraph("Lista del inventario de bienes constatados físicamente:  \n", myfont);
            ...
            Paragraph total = new Paragraph("Total: " + Session["_total"].ToString()+  "\n ", myfont3);
            total.Alignment = Element.ALIGN_RIGHT;
            document.Add(total);

            document.Add(new Paragraph("\n"));

            Paragraph P4 = new Paragraph("Se deja constancia que el receptor del presente documento el señor(a)" + ConfigurationManager.AppSettings["AreaActivosFijosIngreso"].ToString() + ", se encargará de incluir o contratar  la póliza de seguro, de acuerdo con lo que estipula el manual de activos fijos referente a la inclusión de seguros. \n\n", myfont);
            P4.Alignment = Element.ALIGN_JUSTIFIED;
            document.Add(P4);

            //Paragraph P5 = new Paragraph("En consecuencia, por la demostración que antecede y de conformidad  el  señor(a) " + AreaActivos.ToString() + ", entrega a satisfacción al señor(a) " + cust1 + ", quien recibe a satisfacción los activos fijos y bienes sujetos de control administrativo.  ", myfont);
            //P5.Alignment = Element.ALIGN_JUSTIFIED;
            //document.Add(P5);

            Paragraph P6 = new Paragraph("Para constancia de lo actuado y en fe de conformidad y aceptación, suscriben la presente acta de entrega – recepción en 2 ejemplares de igual tenor y efecto las personas que intervienen en esta diligencia.", myfont);
            ...
            tablafirmas.DefaultCell.HorizontalAlignment = 1;
            tablafirmas.WidthPercentage = 100;
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph(ConfigurationManager.AppSettings["AreaActivosFijosIngreso"].ToString(), myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));

            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("CONTROL DE ACTIVOS FJIOS", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("JEFATURA ADMINISTRATIVA FINANCIERA", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));

            tablafirmas.AddCell(new Paragraph("", myfont3));
            //tablafirmas.AddCell(new Paragraph("Entregue Conforme", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
            tablafirmas.AddCell(new Paragraph("", myfont3));
    ...
    protected void btn_Items_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                upgrid.Update();
                pancus.Visible = true;
                pancus.GroupingText = "Items [ " + dt.Rows.Count.ToString() + " Items ]";
    ...
    protected void btnBuscar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
           ...
           // {
            ...
            //query = "AND ACT_FECHACOMPRA between " + rdp_Desde.SelectedDate.Value.Date.ToString("yyyy-MM-dd") +","+ rdp_Hasta.SelectedDate.Value.Date.ToString("yyyy-MM-dd");

            query = query + " AND ACT_FECHACOMPRA between '" + rdp_Desde.SelectedDate.Value.Date.ToString("yyyy-MM-dd")+"' "+ dat +" '"+ rdp_Hasta.SelectedDate.Value.Date.ToString("yyyy-MM-dd") + "'";
           // }

            //query = query + " AND act_fechabaja is null";

            DataTable dt = Logica.HELPER.getReporteINCLUSIONPdf(query, 1);
            Cache["Repodt"] = dt;
            rgitems.DataSource = setExport(dt);
            rgitems.DataBind();
            DataTable dtTotal = Logica.HELPER.getReporteINCLUSIONPdf(query, 2);
            if (dtTotal.Rows[0][0].ToString() == "" || dtTotal.Rows[0][0].ToString() == null)
                Session["_total"] = "0";
            else
                Session["_total"] = double.Parse(dtTotal.Rows[0][0].ToString());
            upgrid.Update();
            upGrillaItems.Update();
            pancus.Visible = true;
            pancus.GroupingText = "Activos [ " + dt.Rows.Count.ToString() + " Items ]";
            Session["_totalItems"] = dt.Rows.Count.ToString();

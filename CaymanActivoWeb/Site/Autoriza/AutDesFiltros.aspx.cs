...
public partial class AutDesFiltros : System.Web.UI.Page
{
    ...
    protected void ibsave_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            DataTable dt = Logica.HELPER.getReporteFiltradoRFID(query);
        ...
        try
        {
            ...
            if (codigoCorrecto())
            {
                if (Logica.HELPER.comprobarCbIngresado(txtbuscb.Text))
                {
                    ...
                    DataTable dt = Logica.HELPER.getReporteFiltradoRFID(" and ACT_CODBARRAS = '" + txtbuscb.Text.Trim() + "'");
        ...
        if (verificaItemsSel())
        {
            ...
            foreach (GridViewRow rw in rgreporte.Rows)
            {
                ...
                if (check.Checked == true)
                {
                    ...
                    err = Logica.HELPER.FltraRFID(rw.Cells[8].Text.Trim(), 0);
                    if (err != "")
                    {
                        conterr++;
                    }
        ...
        if (verificaItemsSel())
        {
            ...
            foreach (GridViewRow rw in rgreporte.Rows)
            {
                ...
                if (check.Checked == true)
                {
                    ...
                    err = Logica.HELPER.FltraRFID(rw.Cells[8].Text.Trim(), 1);
                    if (err != "")
                    {
                        conterr++;
                    }
